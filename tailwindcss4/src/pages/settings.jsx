...
export default function Settings() {
  ...
  return (
    <div className="min-h-screen text-white bg-[radial-gradient(circle_at_top,_#1FAE78_0%,_#123D2B_30%,_#0B1F17_70%)] px-4 py-6">
      ...
      <div className="max-w-2xl mx-auto">
              ...
              <label htmlFor="photoUpload" className="cursor-pointer">
                {photoURL ? (
                  <img
                    src={photoURL}
                    alt="Profile"
                    className="w-16 h-16 rounded-full object-cover ring-2 ring-emerald-400 shadow-[0_0_25px_rgba(31,174,120,0.6)]"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center text-xl font-bold shadow-[0_0_25px_rgba(31,174,120,0.6)]">
        ...
        <Section title="Help & Support">
          <SettingItem icon={HelpCircle} title="Help Center" subtitle="FAQs & guides" onClick={() => navigate("/help")} />
          <SettingItem icon={FileText} title="Report a Problem" subtitle="Tell us what's wrong" onClick={() => setShowReportModal(true)} />
        </Section>

        <Section title="Legal">
          <Link to="/terms"><SettingItem icon={FileText} title="Terms & Conditions" subtitle="Review our terms" /></Link>
          <Link to="/privacy"><SettingItem icon={FileText} title="Privacy Policy" subtitle="How we protect your data" /></Link>
        </Section>
...
const GlassCard = ({ children }) => (
  <div className="mb-6 p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-[0_0_40px_rgba(31,174,120,0.15)]">
    {children}
  </div>
);
...
const SettingItem = ({ icon: Icon, title, subtitle, onClick }) => (
  <button onClick={onClick} className="w-full flex justify-between items-center p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10">
...
const ToggleItem = ({ icon: Icon, title, value, onChange }) => (
  <div className="flex justify-between items-center p-4 rounded-xl bg-white/5 border border-white/10">
    ...
    >
      <div className={`h-5 w-5 bg-white rounded-full transition ${value ? "translate-x-5" : "translate-x-1"}`} />
    </button>
  </div>
);
